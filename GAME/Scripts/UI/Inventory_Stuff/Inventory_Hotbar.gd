...
#connects the ui update signal
func _ready() -> void:
	Global.inventory_updated.connect(_update_hotbar_UI)
	_update_hotbar_UI()

#Empties to hotbar, checks the data saved in the Global Singleton and add them back in.
func _update_hotbar_UI():
	clear_hotbar_container()
	for i in range(Global.hotbar_size):
		var slot = Global.inventory_slot_scene.instantiate()
		slot.set_slot_index(i)
		#Signals for the drag and drop of items
...
#get the slots which the drag and drop are interacting with
...
#moves an item for where it was dragged to where it was dropped.
